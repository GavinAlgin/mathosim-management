...
export default function RegisterForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // 2. Add profile entry (trigger usually handles this, but we update role + username)
      ...
      // 3. Redirect — middleware will take care of role-based routing
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <button
                type="submit"
                disabled={loading}
                className="flex items-center justify-center w-full rounded-md bg-[#00483d] px-3 py-1.5 text-sm font-semibold text-white shadow-sm hover:bg-[#e2fe4f] hover:text-black focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-[#e2fe4f] cursor-pointer"
              >
                {loading ? <Loader2 className="animate-spin h-4 w-4" /> : 'Register'}
              </button>
            </div>
          </form>
        </div>
