...
export function AddEditEventDialog({
  ...
  /**
   * IMPORTANT:
   * useForm MUST use Zod INPUT type
   */
  ...
  const onSubmit = (values: EventFormInput) => {
    try {
      // Parse to OUTPUT (Dates are guaranteed here)
      const parsed: EventFormOutput = eventSchema.parse(values);

      const formattedEvent: IEvent = {
        ...parsed,
        startDate: format(parsed.startDate, "yyyy-MM-dd'T'HH:mm:ss"),
        endDate: format(parsed.endDate, "yyyy-MM-dd'T'HH:mm:ss"),
        id: isEditing ? event.id : Math.floor(Math.random() * 1_000_000),
        user: isEditing
          ? event.user
          : {
