            ...
            >
                {MONTHS.map((month, monthIndex) => {
                    ...
                    return (
                        <motion.div
                            key={month}
                            className="flex flex-col border border-border rounded-lg shadow-sm overflow-hidden bg-card"
                            initial={{ opacity: 0, scale: 0.95 }}
                            animate={{ opacity: 1, scale: 1 }}
                            transition={{ delay: monthIndex * 0.05, ...transition }}
                        >
                            <div
                                className="px-3 py-2 text-center font-semibold text-sm cursor-pointer hover:bg-primary/10 transition-colors"
                                onClick={() => setSelectedDate(new Date(currentYear, monthIndex, 1))}
                            >
                                {month}
                            </div>

                            <div className="grid grid-cols-7 text-center text-[10px] font-medium text-muted-foreground py-1 border-y border-border/50">
                                {WEEKDAYS.map((day) => (
                                    <div key={day} className="p-1">{day}</div>
                                ))}
                            </div>

                            <div className="grid grid-cols-7 gap-px p-1.5 flex-grow text-xs">
                                {cells.map((cell) => {
                                        ...
                                        >
                                            {isCurrentMonth && hasEvents ? (
                                                <EventListDialog date={cell.date} events={dayEvents}>
                                                    <div className="w-full h-full flex flex-col items-center justify-start gap-0.5 cursor-pointer hover:bg-accent/50 rounded-md transition-colors">
                                                        <span className={cn(
                                                ...
                                                <div className="w-full h-full flex flex-col items-center justify-start">
                                                    <span className={cn(
                                                        "size-5 flex items-center justify-center font-medium",
                                                        isToday && isCurrentMonth && "rounded-full bg-primary text-primary-foreground"
                                                    )}>
                                                        {cell.day}
                                                    </span>
                                                </div>
