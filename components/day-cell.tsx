...
// Framer Motion typed transition
...
export function DayCell({ cell, events, eventPositions }: IProps) {
  ...
  const renderEventAtPosition = useCallback(
    (position: number) => {
      ...
      if (!event) return <motion.div key={`empty-${position}`} className="lg:flex-1" initial={false} animate={false} />;
            ...
            {cellEvents.length === 0 && !isMobile ? (
              <div className="w-full h-full flex justify-center items-center group">
                <AddEditEventDialog startDate={date}>
                  <Button variant="ghost" className="border opacity-0 group-hover:opacity-100 transition-opacity duration-200">
